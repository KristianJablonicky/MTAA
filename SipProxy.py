...
class UDPHandler(socketserver.BaseRequestHandler):   
    ...
    def processRegister(self):
        ...
        if rx_invalid.search(contact) or rx_invalid2.search(contact):
            ...
            self.sendResponse("488 Zakázaná adresa")
            return
    ...
    def processInvite(self):
        ...
        if len(origin) == 0 or not origin in registrar:
            ...
            self.sendResponse("400 Zlá žiadosť")
            return
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                logging.info("%s volá používateľovi %s", self.getOrigin(), self.getDestination())
                
                socket.sendto(text.encode("utf-8") , claddr)
            else:
                #self.sendResponse("480 Temporarily Unavailable")
                self.sendResponse("480 Dočastne nedostupné")
        else:
    ...
    def processAck(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar:
                ...
                logging.info("%s odpovedá používateľovi %s", self.getDestination(), self.getOrigin())
                socket.sendto(text.encode("utf-8"),claddr)
    ...
    def processNonInvite(self):
        ...
        if len(origin) == 0 or not origin in registrar:
            #self.sendResponse("400 Bad Request")
            self.sendResponse("400 Nesprávna žiadosť")
            return
        destination = self.getDestination()
        if len(destination) > 0:
            #if registrar.has_key(destination) and self.checkValidity(destination):
            if destination in registrar and self.checkValidity(destination):
                ...
                if rx_bye.search(text):
                    logging.info("%s ukončil hovor s používateľom %s", self.getOrigin(), self.getDestination())
                socket.sendto(text.encode("utf-8") , claddr)    
            else:
                #self.sendResponse("406 Not Acceptable")
                self.sendResponse("406 Nedá sa vykonať Accept")
        else:
            #self.sendResponse("500 Server Internal Error")
            self.sendResponse("500 Interná chyba servera")
...
if __name__ == "__main__":
    logging.basicConfig(format='%(asctime)s - %(message)s',filename='denník hovorov.log',
                        level=logging.INFO,
                        datefmt='%H:%M:%S')
